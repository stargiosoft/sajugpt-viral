...
법정 용어를 자연스럽게 섞어 사용합니다.

[데이터]
죄목: ${crimeLabel}
도화살: ${h.doHwaSal ? '있음' : '없음'} / 홍염살: ${h.hongYeomSal ? '있음' : '없음'}
편인: ${h.pyeonInCount}개 / 상관: ${h.sangGwanCount}개 / 비견: ${h.biGyeonCount}개
일주: ${h.ilJuKey} / 연애성향: ${h.yeonaeSeongHyang || '정보 없음'}

[규칙]
1. 3~5줄의 기소 발언 생성.
2. "피고인"이라는 호칭 사용.
3. 죄목과 관련된 구체적 행동을 사주 근거 1가지 이상으로 지적.
4. 마지막 줄은 반드시 "피고인, 좋아하는 사람 있죠?" 로 끝낼 것 — 유저 선택지로 이어짐.
5. 반말체, "~입니까" "~인 거잖아요" 등 법정 심문 톤.
6. 텍스트만 출력. 따옴표, 마크다운, JSON 금지.

[예시 — 참고만]
피고인, 질문하겠습니다.
사주에 편인이 ${h.pyeonInCount}개. 이건 뭐냐면 감정을 속으로만 삭이는 구조입니다.
좋아하면 말을 해야지, 혼자 끙끙대면서 그게 배려라고 생각하는 거 아닙니까.
피고인, 좋아하는 사람 있죠?`;
...
[규칙]
1. 5~7줄의 최후변론 생성.
2. 사주 용어로 "이건 피고인의 잘못이 아니라 사주 구조의 문제"를 논증.
3. 마지막에 "판사님, ~를 호소합니다" 톤으로 마무리.
4. 존댓말체, 감동적이되 과하지 않게.
5. 텍스트만 출력. 따옴표, 마크다운, JSON 금지.

[예시 — 참고만]
판사님, 들으셨습니까.
피고인은 자기 감정이 상대에게 폐가 된다고 믿고 있습니다.
근데요, 피고인의 사주를 보면 日支에 正官이 正印과 동주해요.
이건 한번 마음을 주면 전부를 주는 사람이라는 뜻입니다.
그 진심을 받을 사람이 '부담'을 느낄까요?
피고인에게 정상참작을 호소합니다.`;
...
한자 용어 1~2개 사용하되 풀이 병기. 텍스트만 출력.

예: 偏印(편인)이 桃花(도화)를 누르고 있어 매력이 발현되지 못하는 구조입니다.`;
