import type { TestCatalogItem } from '@/types/testCatalog';

// 모아모아 홈 화면의 단일 데이터 소스 (기존 ViralHub 인라인 ITEMS 대체)
// participantLabel / isNew / popularityRank는 큐레이션된 정적 데이터.
// 2026-07-15: 귀신타로만 노출하기로 결정 — 나머지는 데이터를 지우지 않고 visibleOnHome만 false로 내림.
// 다시 노출하려면 해당 항목의 visibleOnHome을 true로 되돌리면 됨.
export const TEST_CATALOG: TestCatalogItem[] = [
  {
    id: 'sexy-battle',
    title: '색기 배틀',
    description: '사주로 페로몬 등급 판정, 나한테 꼬인 남자는 몇 명?',
    href: '/sexy-battle',
    emoji: '🔥',
    imageSrc: '/home/thumbnails/sexy-battle.jpg',
    category: 'love',
    colorTheme: 'orange',
    participantLabel: '12.4만',
    isNew: false,
    popularityRank: 2,
    editorPick: true,
    ready: true,
    visibleOnHome: false,
  },
  {
    id: 'autopsy',
    title: '사주 부검실',
    description: '전남친 사주를 부검하고 사망진단서를 발급합니다',
    href: '/autopsy',
    emoji: '🔬',
    imageSrc: '/home/thumbnails/autopsy.jpg',
    category: 'analysis',
    colorTheme: 'graphite',
    participantLabel: '8.9만',
    isNew: true,
    popularityRank: 3,
    ready: true,
    visibleOnHome: false,
  },
  {
    id: 'stock',
    title: '주가 조작단',
    description: '저평가된 연애운, 주가 조작 작전을 세워드립니다',
    href: '/stock',
    emoji: '📈',
    imageSrc: '/home/thumbnails/stock.jpg',
    category: 'money',
    colorTheme: 'slate',
    participantLabel: '6.2만',
    isNew: false,
    editorPick: true,
    ready: true,
    visibleOnHome: false,
  },
  {
    id: 'night-manual',
    title: '밤 설명서',
    description: '사주 기반 체질 평가와 시종들의 난상토론',
    href: '/night-manual',
    emoji: '🌙',
    imageSrc: '/home/thumbnails/night-manual.jpg',
    category: 'analysis',
    colorTheme: 'mist',
    participantLabel: '3.8만',
    isNew: false,
    editorPick: true,
    ready: true,
    visibleOnHome: false,
  },
  {
    id: 'dating-sim',
    title: '데이트 시뮬레이션',
    description: '사주 궁합 기반 AI 캐릭터와 5턴 데이트 대화',
    href: '/dating-sim',
    emoji: '💘',
    imageSrc: '/home/thumbnails/dating-sim.jpg',
    category: 'sim',
    colorTheme: 'graphite',
    participantLabel: '2.9만',
    isNew: true,
    editorPick: true,
    ready: true,
    visibleOnHome: false,
  },
  {
    id: 'ghost-tarot',
    title: '귀신 타로(운세편)',
    description: '당신에게 붙은 귀신이 전하는 이번 달 경고, 봉인된 카드를 뽑아보세요',
    href: '/ghost-tarot',
    emoji: '👻',
    imageSrc: '/home/thumbnails/ghost-tarot.jpg',
    category: 'analysis',
    colorTheme: 'graphite',
    participantLabel: '15.2만',
    shareLabel: '1.1만',
    isNew: true,
    popularityRank: 1,
    sajugptBadge: true,
    ready: true,
    visibleOnHome: true,
  },
  {
    id: 'romance-ghost-tarot',
    title: '귀신 타로(연애편)',
    description: '당신에게 붙은 존재가 전하는 인연의 신호, 카드 5장으로 확인하세요',
    href: '/romance-ghost-tarot',
    emoji: '👻',
    imageSrc: '/home/thumbnails/romance-ghost-tarot.jpg',
    category: 'love',
    colorTheme: 'graphite',
    participantLabel: '8,400',
    shareLabel: '940',
    isNew: true,
    sajugptBadge: true,
    ready: true,
    visibleOnHome: true,
  },
  {
    id: 'deang-saju',
    title: '댕댕사주',
    description: '사주를 강아지로 번역, 나는 무슨 견종일까?',
    href: '/deang-saju',
    emoji: '🐾',
    imageSrc: '/home/thumbnails/deang-saju.jpg',
    category: 'analysis',
    colorTheme: 'orange',
    participantLabel: '6,700',
    shareLabel: '810',
    isNew: true,
    sajugptBadge: true,
    ready: true,
    visibleOnHome: true,
  },
  {
    id: 'love-chat',
    title: '카톡 연애도감',
    description: '카톡 습관만 봐도 연애 스타일이 보인다, 내 연애 카톡 캐릭터는?',
    href: '/love-chat',
    emoji: '💌',
    imageSrc: '/love-chat/thumbnail-v2.png',
    category: 'love',
    colorTheme: 'orange',
    participantLabel: '5,200',
    shareLabel: '690',
    isNew: true,
    ready: true,
    visibleOnHome: true,
  },
  {
    id: 'money-timeline',
    title: '내 돈복 테스트',
    description: '생년월일만 입력하면 평생의 재물운 흐름을 그래프 하나로 확인해요',
    href: '/money-timeline',
    emoji: '💰',
    imageSrc: '/money-timeline/thumbnail-v3.png',
    category: 'money',
    colorTheme: 'orange',
    participantLabel: '3,900',
    shareLabel: '510',
    isNew: true,
    sajugptBadge: true,
    ready: true,
    visibleOnHome: true,
  },
  {
    id: 'oheng',
    title: '인간 사용설명서',
    description: '내 사주 오행 분포로 진단하는 대표 성향과 기운 보완 처방',
    href: '/oheng',
    emoji: '🍵',
    imageSrc: '/oheng/landing-thumbnail.png',
    category: 'analysis',
    colorTheme: 'mist',
    participantLabel: '2,480',
    shareLabel: '312',
    isNew: true,
    sajugptBadge: true,
    ready: true,
    visibleOnHome: true,
  },
  {
    id: 'solo-guide',
    title: '솔로 탈출 지침서',
    description: '내 사주 속 오성 분포로 알아보는 진짜 연애 유형과 솔로 탈출 처방전',
    href: '/solo-guide',
    emoji: '💘',
    imageSrc: '/solo-guide/landing-thumbnail.png',
    category: 'love',
    colorTheme: 'orange',
    participantLabel: '3,200',
    shareLabel: '410',
    isNew: true,
    ready: true,
    // 2026-08-07: 오행 점수/러브타입 분기 로직 수정 + DB 콘텐츠 25종 확인 후 홈 노출 재개
    visibleOnHome: true,
  },
  {
    id: 'loving-season',
    title: '내 연애의 계절은 언제?',
    description: '설레는 사랑이 찾아오는 시기, 사주로 알아보는 나만의 연애 계절',
    href: '/loving-season',
    emoji: '🌸',
    imageSrc: '/loving-season/landing-thumbnail.png',
    category: 'love',
    colorTheme: 'orange',
    participantLabel: '312',
    shareLabel: '48',
    isNew: true,
    sajugptBadge: true,
    ready: true,
    visibleOnHome: false,
  },
  {
    id: 'shinsal-genius',
    title: '신살시리즈: 지니어스 또라이 사주',
    description: '내 사주 속 숨겨진 천재성과 똘끼를 분석해 드려요',
    href: '/shinsal-series/genius',
    emoji: '🧠',
    category: 'analysis',
    colorTheme: 'graphite',
    participantLabel: '186',
    shareLabel: '29',
    isNew: true,
    sajugptBadge: true,
    ready: true,
    visibleOnHome: true,
  },
  {
    id: 'ziwei-chart',
    title: '자미두수 명반',
    description: '나의 운명을 결정짓는 12궁과 별자리를 확인해보세요',
    href: '/ziwei-chart',
    emoji: '✨',
    category: 'analysis',
    colorTheme: 'mist',
    participantLabel: '204',
    shareLabel: '33',
    isNew: true,
    sajugptBadge: true,
    ready: true,
    visibleOnHome: true,
  },
];
