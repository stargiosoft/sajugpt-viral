  ...
  return (
    ...
    >
      <div className="flex flex-col items-center" style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 700, color: C.text, textAlign: 'center', fontFamily: MAIN_FONT }}>
          사주 정보를 입력해 주세요
        </h1>
      </div>
      ...
      <motion.div 
        ...
        style={{ borderRadius: '20px', backgroundColor: C.panelBg, padding: '24px 20px 28px', border: `1px solid ${C.border}` }} 
        initial="hidden" 
        animate="visible" 
      ...
      >
        ...
        <motion.div style={{ marginTop: '32px' }} variants={FADE_UP}>
          <PressableButton 
            onClick={props.isValid ? props.onSubmit : undefined} 
            disabled={!props.isValid} 
            label="결과보기" 
            style={{ height: '56px' }} 
            bgStyle={{ backgroundColor: props.isValid ? C.accent : C.border, borderRadius: '16px', border: 'none' }} 
            hoverBackground={C.accentHover} 
            textStyle={{ color: props.isValid ? C.textOnAccent : C.textTertiary, fontWeight: 700, fontSize: '16px', fontFamily: MAIN_FONT }} 
          />
        </motion.div>
        ...
        {props.error && (
          <motion.div style={{ marginTop: '20px', borderRadius: '10px', padding: '12px 16px', backgroundColor: '#450a0a', border: `1px solid ${C.danger}` }} variants={FADE_UP}>
            <p style={{ color: C.danger, fontSize: '13px', fontFamily: MAIN_FONT }}>{props.error}</p>
          </motion.div>
        )}
      </motion.div>
    </motion.div>
