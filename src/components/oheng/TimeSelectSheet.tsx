...
export default function TimeSelectSheet({ value, unknownTime, onSelect }: Props) {
  ...
  return (
    <>
      ...
      >
        <span style={{ fontSize: '16px', color: selected ? '#2B2013' : '#9C8A63' }}>
          {selected ? (selected.isUnknown ? '모름' : `${selected.label} (${selected.range})`) : '모름'}
        </span>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
          <path d="M6 9l6 6 6-6" stroke="#9C8A63" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>
        ...
        {open && (
          <>
            <motion.div
              ...
              style={{ backgroundColor: 'rgba(43,32,19,0.5)', zIndex: 90 }}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              ...
              >
                <div style={{ width: '36px', height: '4px', borderRadius: '2px', backgroundColor: '#D9C79E' }} />
              </div>
              <p style={{ fontSize: '20px', fontWeight: 700, color: '#2B2013', padding: '26px 20px 14px', flexShrink: 0 }}>
                태어난 시간을 선택해 주세요
              </p>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: '16px',
                        border: 'none',
                        backgroundColor: isSelected ? '#E8D9AE' : 'transparent',
                        cursor: 'pointer',
                      }}
                    >
                      <span
