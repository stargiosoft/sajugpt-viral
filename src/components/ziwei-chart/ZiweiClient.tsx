...
export default function ZiweiClient() {
  ...
  const [step, setStep] = useState<
    'landing' | 'input' | 'analyzing' | 'result'
  >('landing');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const engineResult =
        JamidusuEngine.makePurpleStarTable(dt, sexInt);
  ...
  const handleStart = useCallback(() => {
    console.log('🚀 ZiweiClient handleStart 실행');
    console.log('현재 step:', step);

    setStep('input');
  }, [step]);

  console.log('🎯 현재 ZiweiClient step:', step);
    ...
    >
      <div className="w-full h-full flex flex-col max-w-110 md:max-w-150">
        <div className="flex-1 overflow-auto w-full">
          ...
          {step === 'landing' && (
            <ZiweiLanding onStart={handleStart} />
          )}
