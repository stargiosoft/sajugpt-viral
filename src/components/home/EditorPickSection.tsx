...
export default function EditorPickSection({ items, onSelect }: EditorPickSectionProps) {
  const picks = items.filter((item) => item.editorPick);

  if (picks.length === 0) return null;

  return (
    <div style={{ padding: '62px 0 4px' }}>
      <h2
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 px-4 md:px-6 lg:px-8">
        {picks.map((item) => (
          <TestCard key={item.id} item={item} onSelect={onSelect} />
        ))}
      </div>
    </div>
  );
